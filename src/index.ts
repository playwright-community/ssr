...
class WebServer {
  private _appProcess: import("child_process").ChildProcess;
  private _resolver: RemoteHttpResolverOverWS;
...
class RequestImpl implements PWRequest {
  private _requestPayload: ArrayBuffer;
  private constructor(public readonly _request: InteractiveRequest) {}

  static async create(request: InteractiveRequest): Promise<RequestImpl> {
    const pwRequest = new RequestImpl(request);
    pwRequest._requestPayload = await request.arrayBuffer();
    return pwRequest;
  }
